    ...
    @if($userCanAccess)
        <div class="video-container" 
            ...
            @if(!$isWatching)
                <div class="video-placeholder d-flex align-items-center justify-content-center" 
                     style="background-image: url('{{ $lesson->thumbnail_url ?? asset('images/video-placeholder.jpg') }}')">
        ...
        <script>
            document.addEventListener('livewire:load', function () {
                // Set up heartbeat to track video progress
                let player;
                ...
                window.addEventListener('message', function(event) {
                    ...
                    if (event.data && event.data.event === 'playerReady') {
                        ...
                        // Start heartbeat to track video time
                        heartbeatInterval = setInterval(() => {
                            player.postMessage({ action: 'getCurrentTime' }, '*');
                        }, 5000);
                    }
                    
                    // Handle video time updates
                    if (event.data && event.data.currentTime) {
                        @this.call('heartbeat', event.data.currentTime);
                    }
                    
                    // Handle video completion
                    if (event.data && event.data.event === 'videoEnded') {
